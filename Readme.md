...
## 心得回饋
1. **難得的實習經驗**｜對於本科是經濟系的大學生來說，很幸運可以接觸機器學習研發部門的工作。

2. **教育訓練很有幫助**｜一開始的教育訓練對後續的實習很有幫助，可能因為沒有電資學院的基礎背景，對於環境變數、log、Hint其實都不是很熟，剛好藉此補足一些學校沒教的基礎。除此之外，第一個禮拜也進行了sqlite和api的練習，很幫助我熟悉之後的實習的工作。

3. **專案式學習**｜實習的過程都以專案來進行，這樣的方式比片面式的工作好的許多，可以了解整個專案的過程，專案結束也會將資料整理下來，這種模式也幫助我在需要的時候，可以回頭去了解之前的專案。

4. **探索未知的領域**｜在實習以前，我並沒有接觸過mT5和GPT2（Bert也是學校課程簡單使用過而已）。因此，這次的自動摘要對我來說是一大挑戰，在過程中要了解一些參數的功能和調整參數來進行實驗。在GPT2的部分更需要調整模型才能使用，在這個過程中也讓我更熟debug模式的操作，也建議實習生可以一開始就裝(Pycharm)，應該會比Vscode還要更好用一些。

5. **實習主管都適時給予需要的協助**｜非常感謝RD2的主管Jason、偉帆，在這次的實習過程每當到問題時，主管都會很有耐心的幫我debug，且在專案進行到一個段落的時候會進行code review，幫助我的學習。也感謝部門的柏翰提供我爬蟲的協助，彥鈞給我一些程式學習上的建議。除此之外，在實習的過程中，每當我遇到問題，Jason有時候會先找一些相關的資料或是提供一些關鍵字讓我閱讀，這對我來說收穫很大，也因為這次專案更了解transformers、Huggingface上一些好用的套件，如果沒有Jason的協助，專案的進行一定不會那麼順利。
6. **一些小建議**｜
    - 感覺教育訓練的時間可以再拉長一兩天，尤其是了解一些基本知識的部分。
    - 如果能在專案開始前，就先讓實習生了解這次研發的目的，可以讓實習生更了解這個專案進行的方向，應該會有所幫助。
    - 因為習慣用MacOS系統的關係，實習一開始是用自己的電腦，但在跑模型的時候就會花上許多時間，雖然jupyter notebook 不能debug跟操作有一些困難，但如果能先用jupyter notebook的GPU，可以加速訓練的過程。

最後，再次感謝這次的實習機會，我也在實習過程中收穫滿滿！
